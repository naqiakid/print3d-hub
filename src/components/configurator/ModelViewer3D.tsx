...
export default function STLViewer({
  ...
  const safeUrls = (urlsProp ?? []).filter((u): u is string => typeof u === 'string' && u.trim().length > 0)
  const previewUrls = safeUrls.filter(url => isPreviewFile(url))
  const printableUrls = safeUrls.filter(url => !isPreviewFile(url))
  ...
  const baseUrlsKey = safeUrls.map(url => (url?.split('#')[0] ?? '')).join(',')
  ...
  useEffect(() => {
    ...
    (urls ?? []).filter((u): u is string => typeof u === 'string' && u.trim().length > 0).forEach((urlWithHash, i) => {
