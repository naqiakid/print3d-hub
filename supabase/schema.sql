...
-- ── 6. PRINT PROFILES ───────────────────────────────────────
-- Each owner can have multiple slicer profiles for their printer.
-- A profile captures nozzle size, infill settings, and available add-ons.
...
-- Customers need to read profiles when requesting a print
CREATE POLICY "print_profiles_public_read"
  ON print_profiles FOR SELECT USING (TRUE);

-- Only the printer owner can create/update/delete their profiles
